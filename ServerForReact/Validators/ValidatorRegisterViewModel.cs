...
namespace ServerForReact.Validators
{
    public class ValidatorRegisterViewModel : AbstractValidator<RegisterViewModel>
    {
        ...
        public ValidatorRegisterViewModel(IStudentService studentService)
        {
            ...
            RuleFor(x => x.Email)
                ...
                .DependentRules(() =>
                {
                    RuleFor(x => x.Email).Must(studentService.IsEmailExistRegister).WithName("Email").WithMessage("This email is already registered!");
                });
